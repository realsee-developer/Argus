...
class Trainer:
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if not self.checkpoint_conf.pretrain_camera:
            # ignore vggt's camera head
            invalid_keys = [
                "camera_head.empty_pose_tokens",
                "camera_head.embed_pose.weight",
                "camera_head.pose_branch.fc2.weight",
                "camera_head.pose_branch.fc2.bias",
            ]
            for ik in invalid_keys:
                if ik in model_state_dict:
                    model_state_dict.pop(ik)
        ...
        if self.checkpoint_conf.resume:
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            dataloader = self.train_dataset.get_loader(
                ...
                progress=(self.epoch / self.max_epochs) if self.fast_train else 1.0,
            )
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            exact_epoch = (
                self.epoch
                + float(min(data_iter, limit_train_batches)) / limit_train_batches
            )
        ...
        for i, chunked_batch in enumerate(chunked_batches):
            ...
            with ddp_context:
                ...
                if not math.isfinite(loss.item()):
                    logging.warning(
                        f"Non-finite loss ({loss.item()}) at epoch {self.epoch}, "
                        f"step {self.steps[phase]}. GradScaler will skip this step."
                    )
