...
class DynamicTorchDataset(ABC):
    ...
    def get_loader(self, epoch, progress=1.0):
        # print("Building dynamic dataloader with epoch:", epoch)

        # Set the epoch for the sampler
        self.sampler.set_epoch(epoch)
        self.batch_sampler.update_sample_prob(progress)
...
class DynamicBatchSampler(Sampler):
    ...
    def __init__(
        ...
        self.config_image_num_range = image_num_range
        self.image_num_range = deepcopy(image_num_range)
        ...
        # Cal the expected number of iterations per GPU
        sample_num = len(self.sampler)
        Y = np.floor(self.max_img_per_gpu / self.possible_nums)  # sample_batch_size
        E_Y = np.sum(Y * self.normalized_weights)  # expectation_batch_size
        # E_Y2 = np.sum(Y**2 * self.normalized_weights) # expectation_batch_size^2
        # variance = E_Y2 - E_Y**2
        # std_dev = np.sqrt(variance)
        self.expectation_iters_num = np.ceil(sample_num / E_Y).astype(int)
    ...
    def set_epoch(self, epoch):
        ...
        # Use a separate numpy RNG with a fixed seed (independent of rank)
        # to ensure all GPUs produce the same sequence of image_num / batch_size,
        # which keeps iteration counts synchronized across ranks and prevents
        # NCCL collective communication deadlocks.
        self.np_rng = np.random.RandomState(epoch * 100)

    def update_sample_prob(self, progress):
        # print("update_sample_prob progress:", progress)
        # Update image number range and max_img_per_gpu based on progress
        if progress < 0.5:
            self.image_num_range[1] = 4
        else:
            self.image_num_range[1] = self.config_image_num_range[1]
        ...
        self.image_num_weights = {
            ...
            for num_images in range(
                self.image_num_range[0], self.image_num_range[1] + 1
            )
        }
    ...
    def __iter__(self):
        ...
        while True:
            try:
                # Sample random image number and aspect ratio using rank-synchronized RNG
                # to ensure all GPUs produce the same number of iterations per epoch.
    ...
    def __len__(self):
        ...
        return self.expectation_iters_num
